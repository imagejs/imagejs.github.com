...
(function(){
	imagejs.msg('chromomarkers Localhost v0.1 loaded.'); // to notify via console and div#msg
	// code module
	...
	imagejs.modules[id]={
		...
		start:function(){
			...
			//Remove annotation on cvTop.onlick
			cvTop.onclick=function(evt){ 
				var x = evt.clientX-evt.target.offsetLeft+window.pageXOffset;
				var y = evt.clientY-evt.target.offsetTop+window.pageYOffset;
				var dist;
				var smallestDist = 10000000000 ;
				var i_to_be_removed = -1;
				for(var i =0; i < imagejs.data.segs.length; i++){
					var a = jmat.transpose(imagejs.data.segs[i]);
					for(var j=0; j< a.length; j++){
						var b = a[j];
						dist = distance(x,y,b[1],b[0]);
				...
				var segCoordinates = jmat.transpose(imagejs.data.segs[i_to_be_removed]);
				for(var i=0; i<segCoordinates.length; i++){
					var xy_to_remove = segCoordinates[i];
					var x = xy_to_remove[0];
					var y = xy_to_remove[1];
					imagejs.data.seg[x][y] = 0;
				}
...
function distance(x,y,x1,y1){
	var dist = (x1-x)*(x1-x) + (y1-y)*(y1-y);
	return Math.round(Math.sqrt(dist)*10000)/10000; //Find square root and round to 4 decimals
}